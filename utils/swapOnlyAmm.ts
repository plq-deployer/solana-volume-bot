...
export const getBuyTxWithJupiter = async (
  ...
  try {
    ...
    const quoteResponse = await (
      await fetch(
        `https://ultra-api.jup.ag/order?inputMint=So11111111111111111111111111111111111111112&outputMint=${baseMint.toBase58()}&amount=${amount}&taker=${walletAddress}&slippageBps=${SLIPPAGE}&swapMode=ExactIn`,
      )
    ).json();
    ...
    if (quoteResponse.error || quoteResponse.errorMessage) {
      console.error(
        'buy quote response:',
        quoteResponse.error || quoteResponse.errorMessage,
      );
      return;
    }

    const swapTransaction = quoteResponse.transaction;
    if (!swapTransaction) {
      console.error('No swap transaction found in quote response');
      return;
    }
...
export const getSellTxWithJupiter = async (
  ...
  try {
    ...
    const quoteResponse = await (
      await fetch(
        `https://ultra-api.jup.ag/order?inputMint=${baseMint.toBase58()}&outputMint=So11111111111111111111111111111111111111112&amount=${amount}&taker=${walletAddress}&slippageBps=${SLIPPAGE}&swapMode=ExactIn`,
      )
    ).json();
